...
The SDK handles four things you now own:
...
- Reading route-specific fields off the quote and behaving differently per `type`

If none of those are a problem, read on.
...
- [Swift](/architecture/swift-v2), the default for most pairs
- [MCTP and Fast MCTP](/architecture/mctp), built around Circle's CCTP
- [Wormhole Swap](/architecture/wh-swap)

<Note>
  If you find yourself reimplementing transaction construction per route, this is the point where the SDK pays for itself. `getSwapFromEvmTxPayload` returns the payload without the SDK sending it, and its `_forwarder` object gives you the method name and parameters for a contract-level call.
</Note>
...
<Warning>
  **This page is incomplete.** It needs the following from engineering:

  - The error model for the Quote API. `QuoteError` in the SDK carries a `code` and `data`, but the codes are not documented anywhere and a direct API integrator cannot handle failures without them.
  - Per-route calldata construction, or a decision that we deliberately do not document it and point people at the SDK instead.
  - Whether `shuttle` and `monoChain`, which appear in the SDK's quote options, are supported, internal, or deprecated.
</Warning>