...
<Warning>
  **Needs engineering input.** Rate limits with and without an API key, a recommended poll interval, and whether backoff is expected. Whether webhooks or a websocket exist for swap status outside deposit addresses. How long `INPROGRESS` is normal before something is wrong. The full list of granular `status` values and how they map to `clientStatus`.
</Warning>