...
On EVM, a [custom payload](https://github.com/mayan-finance/swap-sdk/blob/main/src/evm/evmSwap.ts#L365) can be included in calls such as `swapFromEvm`, `getSwapFromEvmTxPayload`, or `getMctpFromEvmTxPayload`.
...
if (
  !['WH', 'MCTP', 'FAST_MCTP', 'SWIFT'].includes(quote.type) ||
  quote.toChain === 'hypercore' ||                               // HyperCore builds its own payload
  (['MCTP', 'FAST_MCTP'].includes(quote.type) && (
    quote.cheaperChain === quote.fromChain ||                    // lock-fee MCTP/FastMCTP reject payloads
    quote.hasAuction                                             // auctioned MCTP/FastMCTP won't forward payloads
  ))
) {
  throw new Error(
    'Payloads are supported on WH, Swift, and with-fee, non-auction MCTP/Fast MCTP (never HyperCore).'
  );
}
...
> **Important**
> - On EVM, payloads are supported on [Wormhole](/architecture/wh-swap), [Swift](/architecture/swift), and on MCTP/Fast MCTP only when the route is with-fee (`quote.cheaperChain !== quote.fromChain`) and non-auction (`quote.hasAuction === false`).
> - MCTP and Fast MCTP reject payloads in lock-fee or auction mode, and HyperCore destinations (`quote.toChain === 'hypercore'`) always ignore caller payloads by building their own fixed payload.
