...
description: "Learn how to attach custom payloads, to Mayan swaps, which routes support them, and how to pass raw payload bytes through the EVM, Solana, and Sui SDK integrations."
sidebarTitle: "Zaps (Custom Payloads)"
"og:image": "https://docs.mayan.finance/images/og-zaps.png"
"twitter:image": "https://docs.mayan.finance/images/og-zaps.png"
---
...
<Frame>
  ![Group 2087330093](/images/Group-2087330093.png)
</Frame>
...
You can attach a custom payload from EVM, Solana, or Sui by passing a `Buffer` or `Uint8Array` to the relevant SDK call. Payload support depends on the chain and the route type, so confirm the quote type and route before attaching one, and make sure your destination logic can decode what you send.
...
To receive only routes that carry a custom payload, pass `payload: 'true'` in the second argument (`quoteOptions`) of `fetchQuote` or `generateFetchQuoteUrl`.
...
const quotes = await fetchQuote(params, { payload: 'true' });
