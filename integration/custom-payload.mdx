---
title: "Zaps (Custom Payloads) "
description: "Learn how to attach Zaps, also known as custom payloads, to Mayan swaps, which routes support them, and how to pass raw payload bytes through the EVM, Solana, and Sui SDK integrations."
---

## Overview

Zaps let you attach a small piece of data to a cross-chain swap. This data travels with the transfer and can be read by your destination contract or backend once the funds arrive.
...
This page explains what Zaps are, why they are useful, and how to include them when calling the Swap SDK.
...
Zaps let you attach additional context to a transfer, such as user metadata, routing parameters, or application-specific instructions for your destination logic. If your integration does not require extra data on the destination chain, you can omit it.

Only specific swap routes support caller-supplied payloads; others will reject or override the bytes.

## Use Zaps in the Swap SDK
...
> **Important**
>
> - Zaps are not supported by all route types provided by Mayan. Therefore, it is important to explicitly indicate that you intend to use custom payloads so the quote service can return only routes that support them.
