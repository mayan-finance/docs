...
if (
  ...
  throw new Error(
    'Payloads are supported on Swift, and with-fee, non-auction MCTP/Fast MCTP.'
  );
}
...
> **Important**
> - On EVM, payloads are supported on [Swift](/architecture/swift), and on MCTP/Fast MCTP only when the route is with-fee (`quote.cheaperChain !== quote.fromChain`) and non-auction (`quote.hasAuction === false`).
