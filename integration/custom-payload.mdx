...
if (
  ...
  (quote.type === 'MCTP' && quote.cheaperChain === 'solana') // lock-fee MCTP won't forward payloads
) {
  throw new Error(
    'Custom payloads only work on non-auction MCTP (with-fee) and non-auction Fast MCTP routes from Solana'
  );
}
...
> **Important**
> - Caller-supplied `customPayload` on Solana is supported on non-auction MCTP (with-fee) and non-auction Fast MCTP routes.
> - Swift v2 supports custom payloads, but `swapFromSolana` does not forward them. To include a payload with Swift v2 on Solana, call `createSwiftFromSolanaInstructions` directly and provide both `customPayload` and `customPayloadAccount`.
