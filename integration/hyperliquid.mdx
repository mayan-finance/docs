...
Bridge/swap from a source chain into a HyperCore USDC balance. This is a normal signed transaction on the **source** chain (so the user needs gas there); the SDK and Mayan relayer handle the rest, crediting the chosen HyperCore balance. Deposits typically credit in around 8 seconds.
...
Move USDC out of a HyperCore balance to an EVM chain. Withdrawals are **gasless**: the user signs an EIP-712 typed-data message instead of sending an on-chain transaction, and the SDK submits it to the Mayan relayer. **No source-chain gas is required.** Withdrawals typically land in around 15 seconds.
