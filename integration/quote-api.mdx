...
| mpsDepositAddress       | String | Present only when `mpsDeposit` was requested and the quote is eligible — a deterministic deposit address on the **source** chain. Sending the input token to it settles the swap via the [Mayan Payment Service](/payment-service/overview), no signature required. See [MPS deposit address](#mps-deposit-address-mpsdeposit). |
...
response carries an extra `mpsDepositAddress` — a deterministic address on the *source* chain. When
the payer simply sends the input token to that address, the [Mayan Payment Service (MPS)](/payment-service/overview)
detects the deposit and settles the swap to `destinationAddress` on the destination chain, with **no
transaction to sign** and no SDK swap call. This is the same address a merchant integration would get
from MPS's [`POST /deposit-address/generate`](/payment-service/api-reference#generate-deposit-address);
here the quote endpoint derives it for you inline.

Requirements and behavior:

- **`destinationAddress` is required.** Without it the field is omitted (the address is keyed to the
  final recipient).
- **`mpsUserId` is optional.** It's an integrator-scoped identity (a `0x`-prefixed hex string whose
  ...
  omitted, it is derived from your `apiKey`, so deposits still bucket by caller.
- **Only eligible quotes get an address.** MPS attaches it to Swift, mono-chain, and *direct* (no
  source swap) Fast MCTP quotes whose source chain and token are indexed by MPS — the same
  [deposit source chains](/payment-service/overview#deposit-source-chains) MPS supports (Solana,
  Ethereum, BSC, Arbitrum, Base). Other quotes in the same response simply omit `mpsDepositAddress`.
- **Best-effort.** If the address can't be derived for any reason, the quote is still returned —
  just without the field. Never treat its absence as an error.
...
[events / webhooks](/payment-service/events-and-webhooks) or by polling
