...
Before performing a swap, request a quote for the token pair. The response returns the available routes and the rate for each.

### API Reference

<Card title="Swagger UI" color="#83E72E" icon="circle-s" horizontal href="https://price-api.mayan.finance/swagger">
</Card>
...
| mpsDeposit | Boolean | No | Default is `false`. When `true`, attaches a [deposit address](#deposit-address-mpsdeposit) (`mpsDepositAddress`) to each eligible quote so a payer can fund the swap with a plain transfer instead of signing a transaction. Requires `destinationAddress`. See [deposit address](#deposit-address-mpsdeposit). |
| mpsUserId | String | No | Integrator-scoped identity that keys the deposit address: a `0x`-prefixed hex string whose value fits in 20 bytes. Only used when `mpsDeposit` is `true`; when omitted it is derived from your `apiKey`. |
...
| minAmountOut | Number | Minimum output amount |
...
| mpsDepositAddress | String | Returned when `mpsDeposit` was requested and the quote is eligible: a deterministic deposit address on the **source** chain (`null` otherwise). Sending the input token to it settles the swap via [deposit addresses](/features/deposit-addresses), no signature required. See [deposit address](#deposit-address-mpsdeposit). |
| mpsIntegratorId | String | The integrator the deposit address was minted under, keyed to your API key. `null` unless `mpsDepositAddress` is set. |
...
| swiftAuctionMode | Number | Swift quotes only. Settlement mode of the order (see note below). |

<Tip>
  **Settlement modes** (Swift quotes only, `type = "SWIFT"`)

  - `2`: The user receives at least `minAmountOut`, typically around `expectedAmountOut` and possibly higher or lower.
  - `3`: Exact-out. The user is guaranteed to receive exactly `expectedAmountOut`.
</Tip>

### Deposit address (`mpsDeposit`)

Pass `mpsDeposit: true` to make a quote **fundable by a plain transfer**. On eligible quotes the response carries an `mpsDepositAddress`, a deterministic address on the _source_ chain, plus `mpsIntegratorId` and `mpsUserId`. When the payer simply sends the input token to that address, Mayan detects the deposit and settles the swap to `destinationAddress` on the destination chain, with **no transaction to sign** and no SDK swap call. This is the recommended way to obtain a [deposit address](/features/deposit-addresses). The quote endpoint derives it for you inline, so there is no separate address-generation call to make.

Requirements and behavior:

- **`destinationAddress` is required.** Without it `mpsDepositAddress` is `null` (the address is keyed to the final recipient).
- **Deposit addresses must be enabled for your API key.** The address is minted under your integrator identity, so pass your `apiKey`; without an enabled key the field comes back `null`. Contact [support@mayan.finance](mailto:support@mayan.finance) to enable it.
...
- **Only eligible quotes get an address.** Mayan attaches it to Swift, mono-chain, and _direct_ (no source swap) Fast MCTP quotes whose source chain and token are indexed for deposit addresses (see [supported tokens](/payment-service/api-reference#source-config)), and only when the amount clears the token's minimum. Other quotes in the same response return `mpsDepositAddress: null`.
...
<Note>
  The [Mayan SDK](https://github.com/mayan-finance/swap-sdk) (v15+) exposes this as `fetchQuote` options `mpsDeposit` and `mpsUserId`, and reads it back as `quote.mpsDepositAddress`. Passing `mpsDeposit` forces the SDK's POST-body request path.
</Note>

Track a deposit made to an `mpsDepositAddress` through detection, settlement, and delivery with the [events stream](/payment-service/events) or by polling [`GET /swaps`](/payment-service/api-reference#list-swaps).
...
<Card title="Swagger UI" color="#83E72E" icon="circle-s" horizontal href="https://price-api.mayan.finance/swagger/#/default/TokensController_getTokens">
</Card>
...
To get the aggregated list of tokens from all chains remove `chain` from the query.
