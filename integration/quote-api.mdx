...
Currently, API access is provided manually. Integrators who wish to obtain an API key should send a request email to [**support@mayan.finance**](mailto:support@mayan.finance). After receiving the request, our team will review it and contact you to complete the process. Once approved, we will generate an **API key** and send it to you via email so you can start using the API