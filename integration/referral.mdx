...
- Fees are collected in the fee manager contract and can be withdrawn using this tool:<br />[https://explorer.mayan.finance/withdraw-fee-swift](https://explorer.mayan.finance/withdraw-fee-swift)
...
- Fees are only distributed if the **bridge is successfully completed** and **includes a swap**.<br />(Example: a bridge from Ethereum → Solana where ETH is swapped to WETH will **not** generate referral fees.)
...
Always pass the **full object** prepared in Step 1.<br />The SDK will automatically determine which wallet to use based on the selected bridge method, source, and destination.
