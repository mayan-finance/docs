...
That ordering is what makes it fast. The user is paid before the cross-chain message is verified, so settlement does not wait on source-chain finality.
...
- Settlement in seconds, because the driver fronts the delivery rather than waiting on the source chain.
