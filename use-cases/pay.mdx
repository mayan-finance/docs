...
Mayan covers both. Stablecoins are its most moved asset, with over **\$8B** moved cross-chain, and Fast MCTP is the route to select for them: it moves stablecoins over Circle's CCTP.
...
**The cost stays low at size.** Select the Fast MCTP route and the transfer moves over Circle's CCTP, which carries up to around \$10M in a single transaction.
