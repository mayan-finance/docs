...
(native + USDC on EVM; SOL/USDC/USDT/WETH/WSOL on Solana) are detected automatically. For any other token, call
...
<Note>
  **SOL vs. WSOL on Solana.** Both are auto-detected, as two separate entries pointing at two
  different balances. `SOL` (`standard: "native"`, address `1111…1111`) is native lamports sent to
  the vault address with a plain system transfer. `WSOL` (`standard: "spl"`, address `So1111…1112`)
  is the wrapped-SOL SPL token held in the vault's token account. Match the entry to what you
  actually send — and pass that same `address` to
  [`POST /request-index`](#request-index) if you ever need to trigger one manually.
</Note>
...
<Info>
  ...
  [deposit_detected](/payment-service/events#deposit_detected).
</Info>
