...
explains why (`below_min`, `no_adapter`, `enqueue_error`) — so a dust/spam deposit that will never
produce a swap is an explicit signal, not a detection with no follow-up.
