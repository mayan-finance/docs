...
The API key is used for the [MPS panel](https://mps.mayan.finance) and the event WebSocket. Getting the deposit address from a quote works **without** one. Need a key? Email [support@mayan.finance](mailto:support@mayan.finance), or try the shared demo key `03b9969f-7c6a-4bd1-9ad4-c0e0e9f3dcb7` (for evaluation only).
...
<Note>
  ...
  swapping → completed`; Solana runs `pending_settle → settling → completed`. See [Swap Statuses](/payment-service/statuses) for the full lifecycle and the `error` field.
</Note>
