---
title: "Examples"
description: "Complete, runnable end-to-end examples — deposit ~5 USDC and watch it settle to the destination over the WebSocket."
---

These are full, copy-paste examples: get a deposit address from a quote, have a user send **~5 USDC**,
and watch it settle to the destination over the WebSocket.

## Setup

```bash
export MAYAN_API_KEY=your-api-key                  # for the MPS panel & event tracking
...
The API key is used for the [MPS panel](https://mps.mayan.finance) and the event WebSocket — getting
the deposit address from a quote works **without** one. Need a key? Email
[support@mayan.finance](mailto:support@mayan.finance).
...
export async function getMpsDepositAddress(opts: {
  ...
  apiKey: string;             // your Mayan API key (used for event tracking; the quote works without it)
  ...
  if (!address) {
    throw new Error(
      "No mpsDepositAddress in quotes — is the amount above the token's minimum deposit?",
    );
  }
...
The user sends ~5 USDC to the quote's deposit address on Base; it's settled as USDC to your
...
console.log(`\n➡️  Send ~5 USDC to this address on BASE:\n   ${depositAddress}\n`);
...
~5 USDC to it on Solana; it's settled as USDC to your destination wallet on Arbitrum.
...
console.log(`\n➡️  Send ~5 USDC to this vault on SOLANA:\n   ${depositAddress}\n`);
console.log("Waiting for the deposit and settlement (Ctrl-C to stop)...\n");
