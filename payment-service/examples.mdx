...
Need a key? Email [support@mayan.finance](mailto:support@mayan.finance) and ask to have **deposit addresses enabled** on it — the quote returns `mpsDepositAddress` only for MPS-enabled keys.
