...
- **Mayan decides eligibility, not the integrator.** There is no parameter to pass and no allowlist to join. If a quote comes back as mode `3`, Guaranteed Price is available on that order.
