...
- **A signature, not a transaction.** This is the whole mechanism. The user authorises the order off-chain and the relayer bears the cost of putting it onchain.
...
- **Refunds go back to the signer.** A gasless Swift order requires the refund address to match the wallet that signed. `swiftRefundAddress` is rejected on a gasless quote, so request `gasless: false` if you need a refund to land somewhere else. See [advanced](/build/sdk/advanced).
...
**Consumer flows.** Any interface where explaining gas to the user is a cost in itself. The fewer prerequisites between intent and execution, the fewer people drop out.
