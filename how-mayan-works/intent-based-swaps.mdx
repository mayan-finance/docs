...
A cross-chain move traditionally asks the user to make a series of decisions before anything happens. With an intent, none of them reach the user:

- No route to choose, and no bridge to find.
- No sequence of transactions to get right. One signature covers the whole move.
- No wrapped asset to unwind on the other side.
- No native gas on the destination chain before they can use what arrives.
...
Three transfer methods sit underneath, and the order is matched to one of them: [Swift](/architecture/swift-v2), [MCTP and Fast MCTP](/architecture/mctp), and [Wormhole Swap](/architecture/wh-swap). Selecting per order is what lets one integration serve a small trade and a large one equally well.
