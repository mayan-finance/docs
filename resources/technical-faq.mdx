...
Swift is the fastest path and covers most swaps. MCTP routes over Circle's CCTP and suits high-value stablecoin transfers. Wormhole Swap uses the Wormhole Token Bridge for low-slippage transfers of assets like SOL and ETH. See [Swift](/architecture/swift-v2), [MCTP](/architecture/mctp) and [Wormhole Swap](/architecture/wh-swap).

### How large a swap can Mayan handle?

Single transfers route up to \$10M.
...
Yes. Attach a payload to the swap and your destination logic acts on it as the funds arrive. Request a quote with `quoteOptions: { payload: "true" }`. See [Zaps](/integration/custom-payload).
...
On Swift quotes only, using `swiftRefundAddress`. Request the quote with `gasless: false`, since gasless Swift orders require the refund address to match the signer. See [advanced](/build/sdk/advanced).
